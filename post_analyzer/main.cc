...
public:
  ...
  void FillHistograms(const triplet& trip, unsigned int n_triplets, bool mvaCut);
...
MyAnalysis::MyAnalysis(const Config& config) : Analyse(), currun(0), curlumi(0),
  ...
  //mvaTree(fakeRateFuncsNormal, "mvaTree", "MVA Tree")
{
      ...
      if(!ENABLE_MATCHING || !EARLY_MATCHING || haveEarlyMatch) 
	{
	  ...
	  if(VERTEX_REGIONS != 0)
	      ...
	      for(unsigned int j = 0; j < NumDiTaus(); ++j)
		{
		  ...
		  // second tau
		  for(unsigned int k = 0; k < NumDiTaus(); ++k) if(j != k)
		    {
...
int main(int argc, char* argv[])
{
  ...
  if(doDebug) cout<<"AT THE LOOP POINT ---"<< endl;
  ana.Loop();///1, 2000);
  // ana.Loop(1,20);
  ana.tree->Write();
  if(doDebug) cout<<"TREE IS BEIGN WRITTEN ---- "<< endl;  
  ana.file->Write();
  ana.file->Close();
} //int main(int argc, char* argv[])
...
void MyAnalysis::FillHistogramsFromDoublets(const std::vector<DiTau>& doublets)
{
  ...
  if(ONLY_BEST_DOUBLET && best_doublet) FillHistograms(*best_doublet);//, doublets.size());
}
...
void MyAnalysis::FillHistograms(const triplet& trip, unsigned int n_triplets, bool mvaCut)
{
  ...
  const TauCand& OSTau        = *trip.OStau;
  const TauCand& SSTau        = *trip.SStau;
 
  const TLorentzVector tautau = OSTau.p4 + SSTau.p4;
  const double Lt             = OSTau.Pt() + SSTau.Pt() + mu.Pt();
  ...
  if(mvaCut) cutflow.Pass(cutCombinatorics, trip.region);
...
//    std::cerr << "WARNING!!! Event with no SVfit mass, run=" << Run() << ", lumi=" << LumiBlock() << ", event=" << Number() << ": " << NumMuTauTauPairs() << " di tau pair candidates. Fakes: " << mu_fake << ", " << leadtau_fake << ", " << subtau_fake << std::endl;
  ...
  // Muon
  vMuPt.fill(mu.Pt(), weight, infos);
  vMuEta.fill(mu.Eta(), weight, infos);
  vMuPhi.fill(mu.Phi(), weight, infos);
  vMuIso.fill(mu.PFIsoR4RelDB(), weight, infos);
  // Tau
  ...
  vTauTauDPhi.fill(trip.mvaVars.mvaVars.deltaPhiDiTau, weight, infos);
  vTauTauDEta.fill(trip.mvaVars.mvaVars.deltaEtaDiTau, weight, infos);
  vTauTauDR.fill(trip.mvaVars.mvaVars.deltaRDiTau, weight, infos);
  vTauTauPtRatio.fill(trip.mvaVars.mvaVars.ptRatio, weight, infos);
  vTauTauAngle.fill(trip.mvaVars.mvaVars.angle, weight, infos);
  vTauTauSVfitMass.fill(SVfitMass, weight, infos);
  // Tau Tau Muon
  vMETMuTransMass.fill(calcMt(MET().Px(), MET().Py(), mu.Px(), mu.Py()), weight, infos);
  vLt.fill(Lt, weight, infos);
  ...
  pLeadTauPtDeltaR.fill(Variable<TProfile>::Filler(LeadTauPt, trip.mvaVars.mvaVars.deltaRDiTau), weight, infos);
  pLeadTauPtPtRatio.fill(Variable<TProfile>::Filler(LeadTauPt, trip.mvaVars.mvaVars.ptRatio), weight, infos);
  pSubTauPtMET.fill(Variable<TProfile>::Filler(SubTauPt, MET().Pt()), weight, infos);
  pSubTauPtDeltaR.fill(Variable<TProfile>::Filler(SubTauPt, trip.mvaVars.mvaVars.deltaRDiTau), weight, infos);
  pSubTauPtPtRatio.fill(Variable<TProfile>::Filler(SubTauPt, trip.mvaVars.mvaVars.ptRatio), weight, infos);
  pMETDeltaR.fill(Variable<TProfile>::Filler(MET().Pt(), trip.mvaVars.mvaVars.deltaRDiTau), weight, infos);
  pMETPtRatio.fill(Variable<TProfile>::Filler(MET().Pt(), trip.mvaVars.mvaVars.ptRatio), weight, infos);
  pDeltaRPtRatio.fill(Variable<TProfile>::Filler(trip.mvaVars.mvaVars.deltaRDiTau, trip.mvaVars.mvaVars.ptRatio), weight, infos);

  // MVA Tree
  if(!mvaCut)
    mvaTree.fill(trip.mvaVars, mvaValue, weight);

  if(!mvaCut && (trip.region & def.SIGNAL))
  {
    run = Run();
    lumi = LumiBlock();
    event = Number();
...
//   vLt.fill(Lt, weight, infos);
  ...
  //vBDT8_v1.fill(mvaEvaluatorBDT8_v1.mvaValue(mvaVars.mvaVars), weight, infos);
  // vBDT8_v2.fill(mvaEvaluatorBDT8_v2.mvaValue(trip.mvaVars.mvaVars), weight, infos);
  //vBDT8_v3.fill(mvaEvaluatorBDT8_v3.mvaValue(mvaVars.mvaVars), weight, infos);
  //vBDTG_v2.fill(mvaEvaluatorBDTG_v2.mvaValue(mvaVars.mvaVars), weight, infos);
  //vBDT8Mt_v2.fill(mvaEvaluatorBDT8Mt_v2.mvaValue(mvaVars.mvaVars), weight, infos);
  //vBDTGMt_v2.fill(mvaEvaluatorBDTGMt_v2.mvaValue(mvaVars.mvaVars), weight, infos);
...
//   pDeltaRPtRatio.fill(Variable<TProfile>::Filler(trip.mvaVars.mvaVars.deltaRDiTau, trip.mvaVars.mvaVars.ptRatio), weight, infos);
  ...
  // MVA Tree
//   if(!mvaCut)
//     mvaTree.fill(trip.mvaVars, weight);
} // FillHistograms(const triplet& trip, unsigned int n_triplets, bool mvaCut)
